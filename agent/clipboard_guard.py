...
# ---- Screenshot/Image guard: สแกนรูปในคลิปบอร์ด (เช่น สกรีนช็อต) ด้วย AI Vision ----
try:
...
def clip_image_datauri():
    """ถ้าคลิปบอร์ดมีรูป (สกรีนช็อต) → คืน (data_uri, signature) มิฉะนั้น (None, None).
    ย่อขนาด + แปลง JPEG เพื่อลดขนาดส่งและต้นทุน AI."""
    if not IMAGE_GUARD:
        return None, None
    try:
    ...
    if not isinstance(img, Image.Image):
        return None, None  # ไม่ใช่รูป (อาจเป็นไฟล์/ข้อความ)
    ...
    raw = buf.getvalue()
    return "data:image/jpeg;base64," + base64.b64encode(raw).decode(), hashlib.md5(raw).hexdigest()


def _file_to_datauri(path: str) -> str:
...
def run_gui() -> None:
    ...
    def handle_image(data_uri: str):
        app_title, _ = foreground_app()
        res = inspect_remote("", channel="desktop", action_type="screenshot",
                             destination=app_title, images=[data_uri])
        if not res:
            return
        dec = res["decision"]
        if dec in ("block", "redact"):
            set_clip(CLEARED_NOTICE)  # แทนรูปในคลิปบอร์ดด้วยข้อความ → วางรูปไม่ได้
            popup(res)
        elif dec == "warn":
            popup(res)
        print(f"  [image:{dec}] {res['classification']['label']} risk={res['classification']['risk_score']}")
    ...
    def poll():
        ...
        # สกรีนช็อต/รูปในคลิปบอร์ด → สแกนด้วย AI Vision
        if IMAGE_GUARD:
            uri, sig = clip_image_datauri()
            if uri and sig != state["last_img"]:
                state["last_img"] = sig
                try:
                    handle_image(uri)
                except Exception as e:
                    print("  [image handle error]", e)
    ...
    print(f"   กันสกรีนช็อต/รูป: {'เปิด ✓' if IMAGE_GUARD else 'ปิด (ติดตั้ง pillow เพื่อเปิด)'}")
...
def imgtest(path: str) -> int:
    """ทดสอบสแกนรูป (ไฟล์ภาพ) — พิสูจน์ตัวกันสกรีนช็อต."""
    print(f"เซิร์ฟเวอร์: {'ออนไลน์' if backend_alive() else 'ออฟไลน์'}")
    res = inspect_remote("", channel="desktop", action_type="screenshot", images=[_file_to_datauri(path)])
    if not res:
        print("ประเมินไม่สำเร็จ (เซิร์ฟเวอร์/โมเดล Vision)")
        return 1
    c = res["classification"]
    print(f"\nการตัดสิน : {res['decision'].upper()}")
    print(f"ระดับ     : {c['label']} | ความเสี่ยง {c['risk_score']}/100 | AI ใช้: {c.get('ai_used')}")
    print(f"เหตุผล    : {c['reasons'][:3]}")
    return 0
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--imgtest":
        if len(sys.argv) < 3:
            print("ใช้: python clipboard_guard.py --imgtest <ไฟล์รูป>")
            return 1
        return imgtest(sys.argv[2])
    run_gui()
    return 0
